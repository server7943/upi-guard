...
def detect():
    try:
        ...
        # Calculate age in years correctly
        v7 = int((trans_datetime - dob).days / 365.25)
        v8 = float(request.form.get("trans_amount"))
        v9 = int(request.form.get("state"))
        v10 = int(request.form.get("zip"))
        x_test = np.array([v1, v2, v3, v4, v5, v6, v7, v8, v9, v10], dtype=float)
        ...
        return render_template('result.html', OUTPUT='ERROR: {}'.format(str(e)))
